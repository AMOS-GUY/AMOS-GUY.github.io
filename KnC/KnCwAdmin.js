document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if Amos is losing and handle special condition
    function checkAmosCondition(winner) {
        // Check if the loser is named "Amos" (case insensitive)
        const loser = winner === 'X' ? 'O' : 'X';
        const loserName = gameMode === 'pvc' && loser === 'O' ? 'Computer' : playerNames[loser];
        
        if (loserName.toLowerCase() === 'amos') {
            // Amos doesn't lose - it's a draw!
            messageElement.textContent = "The game developer doesn't lose, the game draws!";
    ...
    function endGame(isDraw) {
        ...
        if (isDraw) {
            ...
            // Check if Amos is losing
            if (checkAmosCondition(currentPlayer)) {
                return; // Special condition handled, exit early
            }
